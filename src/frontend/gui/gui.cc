...
integrator_t integrators[] = {
    ...
    , { "Pathtracer (MIS Diffuse)"  , new xtcore::integrator::pathtracer_is::Integrator() }
    , { "Pathtracer (MIS Full)" , new xtcore::integrator::pathtracer_mis_full::Integrator() }
...
void mm_resolution (workspace_t *ws)
{
    ...
    if (ImGui::BeginMenu("Surface")) {
        ...
        const xtcore::render::resolution_preset_t *resolutions = xtcore::render::resolution_presets(res_entries);
        ImVec2 bd(100,0);
        ...
        ImGui::BeginChild("LST_RES", ImVec2(300, 500), true);
        {
            ...
            for (size_t i = 0; i < res_entries; i++) {
                const xtcore::render::resolution_preset_t *r = &resolutions[i];
                char label[32];
