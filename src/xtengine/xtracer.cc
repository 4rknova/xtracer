...
int main(int argc, char **argv)
{
    ...
    int diff = src.width() * src.height();

    for (int i = 0 ; i < 50; ++i) {
        Pixmap temp = dst;
        printf("evolution %d %d\n", i, diff);

        int j = 0;
        int tdiff = diff;
        while (tdiff >= diff) {
            tdiff = NImg::Operator::diff(src, temp, 0.9);
...
/*
	        ...
	        NImg::IO::Export::ppm_raw(str.append("test.ppm").c_str(), temp);
*/
        }
        diff = tdiff;
        dst = temp;
	...
	NImg::IO::Export::ppm_raw("test.ppm", dst);
