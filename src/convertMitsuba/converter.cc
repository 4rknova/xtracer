...
// Returns a version token (e.g. "v3", "v1.2") if the stem ends with [_-]v<digits>...
// Used to pull the version out of the XML filename when the source is a zip.
static std::string stem_version(const std::string& stem) {
    for (int i = (int)stem.size() - 1; i >= 2; --i) {
        if ((stem[i-1] == '_' || stem[i-1] == '-') &&
            (stem[i] == 'v' || stem[i] == 'V') &&
            std::isdigit((unsigned char)stem[i+1])) {
            return stem.substr(i);   // e.g. "v3"
        }
    }
    return "";
}

static std::string fmt_stem(const std::string& path) {
    std::string s = path_stem(path);
    for (char& c : s) if (c == '-' || c == '_') c = ' ';
    return s;
}
...
class Converter {
    ...
    std::string resource_dir() const {
        ...
        if (!opts_.scene_name.empty()) base += opts_.scene_name + "/";
        return base;
    }
    ...
    std::string rewrite_texture(const std::string& src) {
        ...
        for (auto& r : *resources_)
            if (r.src_path == abs && r.type == "texture") return resource_dir() + fname;
        resources_->push_back({abs, "texture", fname});
        return resource_dir() + fname;
    }
    ...
    std::string rewrite_geometry(const std::string& src) {
        ...
        std::string abs   = path_join(opts_.xml_dir, src);
        for (auto& r : *resources_)
            if (r.src_path == abs && r.type == "geometry") return resource_dir() + fname;
...
public:
    ...
    bool convert(pugi::xml_document& doc, std::ostream& out) {
        ...
        // --- write output ---
        NCFWriter w(out);
        w.comment("Generated by convertMitsuba");
        if (!opts_.comment.empty()) {
            std::string c = opts_.comment;
            for (size_t pos = 0; (pos = c.find("\\n", pos)) != std::string::npos; )
                c.replace(pos, 2, "\n");
            std::istringstream css(c);
            std::string line;
            while (std::getline(css, line)) w.comment(line);
        }
        w.blank();

        // Title: zip mode → format(zip_stem) + version_from_xml; xml mode → format(xml_stem)
        std::string title;
        if (!opts_.source_path.empty()) {
            title = "mitsuba - " + fmt_stem(opts_.source_path);
            std::string ver = stem_version(path_stem(opts_.input_path));
            if (!ver.empty()) title += " " + ver;
        } else {
            title = fmt_stem(opts_.input_path);
        }
        w.kv("title",   title);
        w.kv("version", "1.0");
        w.blank();
