...
required_files=(
  ...
  "wasm_adapter.js"
  "wasm_worker.js"
  "app/data/integrators.json"
  "app/data/sidebar_cards.json"
  "app/data/ftue_steps.json"
  "resolutions.json"
  "xtracer_wasm.js"
  "xtracer_wasm.wasm"
)
...
for f in "${required_files[@]}"; do
  if [[ ! -f "$WEB_DIR/$f" ]]; then
    ...
    echo "  emcmake cmake -S . -B build/intermediate/build-wasm -DXTRACER_ENABLE_WEB=OFF -DXTRACER_ENABLE_WASM=ON"
...
mkdir -p "$DIST_DIR/app/data"
cp "$WEB_DIR/app/data/integrators.json" "$DIST_DIR/app/data/"
cp "$WEB_DIR/app/data/sidebar_cards.json" "$DIST_DIR/app/data/"
cp "$WEB_DIR/app/data/ftue_steps.json" "$DIST_DIR/app/data/"
