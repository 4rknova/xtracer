...
## Threading Constraint Relevant To Realtime GL

- `action::render()` currently spawns detached worker thread for `workspace_t::render()`.
- OpenGL context is created/bound in GUI main thread.
- Any real OpenGL integrator must account for this mismatch explicitly.
