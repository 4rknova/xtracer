...
void ring(object_t *obj, size_t resolution)
{
    if (!obj) return;

    const size_t seg = (resolution < 24 ? 24 : resolution);

    // Cylindrical metallic band (open top/bottom) for classic cardioid caustics.
    const float outer_r = 1.0f;
    const float inner_r = 0.84f;
    const float half_h = 0.32f;
    ...
    for (size_t i = 0; i <= seg; ++i) {
        ...
        append_vertex(obj, nmath::Vector3f(outer_r * ca, -half_h, outer_r * sa), n, u, 0.0f);
        append_vertex(obj, nmath::Vector3f(outer_r * ca,  half_h, outer_r * sa), n, u, 1.0f);
    }
    ...
    for (size_t i = 0; i <= seg; ++i) {
        ...
        const nmath::Vector3f n(-ca, 0.0f, -sa);
        append_vertex(obj, nmath::Vector3f(inner_r * ca, -half_h, inner_r * sa), n, u, 0.0f);
        append_vertex(obj, nmath::Vector3f(inner_r * ca,  half_h, inner_r * sa), n, u, 1.0f);
    }
    ...
    for (size_t i = 0; i < seg; ++i) {
        const int o0 = (int)(base_outer + i * 2);
        const int o1 = o0 + 1;
        const int o2 = o0 + 2;
        const int o3 = o0 + 3;

        append_triangle(out, o0, o2, o3);
        append_triangle(out, o0, o3, o1);

        const int in0 = (int)(base_inner + i * 2);
        const int in1 = in0 + 1;
        const int in2 = in0 + 2;
        const int in3 = in0 + 3;

        append_triangle(out, in0, in3, in2);
        append_triangle(out, in0, in1, in3);
